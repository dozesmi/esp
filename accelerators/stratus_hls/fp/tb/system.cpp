...
void system_t::config_proc()
{
    ...
    load_memory();
    {
        ...
        /* <<--params-->> */
        config.fp_len = fp_len;
        config.fp_n = fp_n;
        config.fp_vec = fp_vec;
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - fp");
        ...
        ESP_REPORT_TIME(end_time, "END - fp");
...
void system_t::load_memory()
{
...
#else
    in_words_adj = round_up(fp_len * fp_vec, DMA_WORD_PER_BEAT);
    out_words_adj = round_up(fp_vec, DMA_WORD_PER_BEAT);
#endif

    in_size = in_words_adj * (fp_n);
    out_size = out_words_adj * (fp_n);

    in = new int64_t[in_size];
    double_in = new double[in_size];
    for (int i = 0; i < fp_n; i++)
        for (int j = 0; j < fp_len * fp_vec; j++)
            if(j%2 == 0)
    ...
    gold = new double[out_size];
    int_gold = new int64_t[out_size];
    for (int i = 0; i < fp_n; i++)
        for (int j = 0; j < fp_vec; j++) {
            gold[i * out_words_adj + j] = 0;
            for (int k = 0; k < fp_len; k++) {
                gold[i * out_words_adj + j] += double_in[i * in_words_adj + j * fp_len + k];
            }
        }

    memcpy((void *)int_gold, (void *)gold, sizeof(int64_t)*out_size);
...
int system_t::validate()
{
    ...
    for (int i = 0; i < fp_n; i++)
        for (int j = 0; j < fp_vec; j++){
            if (int_gold[i * out_words_adj + j] != out[i * out_words_adj + j])
                errors++;
        }
